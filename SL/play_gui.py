"""
SL/play_gui.py — Play against the supervised-learning trained model (GUI).

The AI picks moves using the policy head directly (no MCTS search),
masked to legal moves, with optional temperature for variety.

Usage:
    python SL/play_gui.py                                          # latest checkpoint
    python SL/play_gui.py --color black                            # play as Black
    python SL/play_gui.py --checkpoint SL/checkpoints/sl_best.pt
    python SL/play_gui.py --temperature 0.5
"""
...
# ═══════════════════════════════════════════════════════════════════════════
#  AI move selection (policy head only)
# ═══════════════════════════════════════════════════════════════════════════
...
def ai_select_move(net, board: chess.Board, device: str,
    ...
    if move not in board.legal_moves:
        # Fallback (should not happen with masking)
...
class PlayGUI:
    def __init__(self, net, human_is_white: bool, temperature: float,
                 device: str):
        self.net = net
        ...
        pygame.display.set_caption("AlphaZero SL — Policy Net")
    ...
    def draw_panel(self):
        ...
        title = self.title_font.render("AlphaZero SL", True, ACCENT)
        ...
        self.screen.blit(self.panel_font.render(
            f"AI:  {ai}  (policy net, T={self.temperature})", True, MUTED),
            (px, py)); py += 30
...
def main():
    ...
    if ckpt and os.path.isfile(ckpt):
        # Infer architecture from checkpoint so it works regardless of config
        ...
        print(f"  Checkpoint arch: {num_filters} filters, {num_blocks} blocks, "
